<?php
	...
	if(isset($_SESSION['user']))
	{
		...
		try
		{
			...
			else
			{
				...
				if($flag == true)
				{
					...
					for($i = 0; $i < sizeof($wyniki); $i++)
					{
						if($wynik > $wyniki[$position])
						{
							...
							$sql = "delete from flappyShip where user ='$name'";
							$polaczenie->query($sql);
							$sql = "insert into flappyShip (id, user, wynik) values ('$i','$name', '$wynik')";
							$polaczenie->query($sql);
						}
					}
				}
				...
				{
					$sql = "insert into flappyShip (id, user, wynik) values ('$count' , '$name', '$wynik')";
	...
	header("Location: https://www.rafalhudaszek.pl/panel_gier/flappyShip/flappyScore.php");
?>